...
geminicassandra query -q "select name, sample_id, family_id, maternal_id, paternal_id from samples" --test-mode -db $cassandra_ips -ks test_query_db | head > obs
check obs exp
rm obs exp
...
geminicassandra query -q "select chrom, start, end, ref, alt, gene, gts_1094PC0018 \
                 from variants \
                 where gene == 'SCNN1D'" --test-mode -db $cassandra_ips -ks test_query_db | head > obs
check obs exp
rm obs exp
...
geminicassandra query -q "select chrom, start, end, ref, alt, gene, gts_1094PC0018, gt_types_1094PC0018 \
                 from variants \
                 where gene == 'SCNN1D'" --test-mode -db $cassandra_ips -ks test_query_db | head -5 > obs
check obs exp
rm obs exp
...
geminicassandra query -q "select chrom, start, end, ref, alt, gene, gts_1094PC0018, gt_types_1094PC0018 \
                 from variants \
                 where gene == 'SCNN1D'" \
             --gt-filter "gt_types.1094PC0018 != HET" --test-mode -db $cassandra_ips -ks test_query_db | head -5 \
       > obs
check obs exp
rm obs exp
...
geminicassandra query -q "select chrom, start, end, ref, alt, gene, gts_1094PC0018, gts_1094PC0019 \
                 from variants" \
             --gt-filter "gt_types.1094PC0018 == HET && gt_types.1094PC0019 == HOM_REF" --test-mode -db $cassandra_ips -ks test_query_db > obs
check obs exp
rm obs exp
...
geminicassandra query --header --show-samples -q "select chrom, start, end, ref, alt \
                                        from variants" --test-mode -db $cassandra_ips -ks test_query_db | head -6 > obs
check obs exp
rm obs exp
...
geminicassandra query -q "select chrom, start,end, ref,alt,gts_1094PC0018 from variants" --test-mode -db $cassandra_ips -ks test_query_db | head \
       > obs
check obs exp
rm obs exp
...
geminicassandra query -q "select chrom, start, end, ref, alt, gene, gt_types_1094PC0019 \
                 from variants" \
             --gt-filter "gt_types.1094PC0019 == HOM_ALT" --test-mode -db $cassandra_ips -ks test_query_db | head \
       > obs
check obs exp
rm obs exp
...
geminicassandra query --header -q "select chrom, start, end, ref, alt, aa_length, gene \
                 from variants" --test-mode -db $cassandra_ips -ks test_query_db | head > obs
check obs exp
rm obs exp
