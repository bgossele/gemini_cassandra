...
gemini query -q "select * from samples" --test-mode -ks test_query_db | head > obs
check obs exp
rm obs exp
...
gemini query -q "select chrom, start, end, ref, alt from variants" --test-mode -ks test_query_db | head \
       > obs
check obs exp
rm obs exp
...
gemini query -q "select chrom, start, end, ref, alt, gene, gts_1094PC0018, gt_types_1094PC0018 \
                 from variants \
                 where gene == 'SCNN1D'" \
             --gt-filter "gt_type.1094PC0018 != HET" --test-mode -ks test_query_db | head -5 \
       > obs
check obs exp
rm obs exp
...
gemini query -q "select chrom, start, end, ref, alt, gene, gts_1094PC0018, gts_1094PC0019 \
                 from variants \
                 where gene == 'SCNN1D'" \
             --gt-filter "gt_type.1094PC0018 == HET || gt_type.1094PC0019 == HOM_REF" --test-mode -ks test_query_db | head -5 \
       > obs
check obs exp
rm obs exp
...
gemini query -q "select chrom, start, end, ref, alt, gene, gts_1094PC0018, gts_1094PC0019 \
                 from variants" \
             --gt-filter "gt_type.1094PC0018 == HET && gt_type.1094PC0019 == HOM_REF" --test-mode -ks test_query_db > obs
check obs exp
rm obs exp
...
gemini query -q "select chrom, start,end, ref,alt,gts_1094PC0018 from variants" --test-mode -ks test_query_db | head \
       > obs
check obs exp
rm obs exp
...
gemini query -q "select chrom, start, end, ref, alt, gene, gt_types_1094PC0019 \
                 from variants" \
             --gt-filter "gt_type.1094PC0019 == HOM_ALT" --test-mode -ks test_query_db | head \
       > obs
check obs exp
rm obs exp
