...
\\Since its launch, HBase has adopted several secondary indexing mechanisms. For indexing, HBase also uses LSM trees\cite{borthakur2011apache}\cite{sears2012blsm}, and it also provides Bloom filters\cite{hbase_schema}. HBase comes with a Java API but without an SQL-like advanced querying language, though a workaround via Apache Hive, another data warehousing and querying project\cite{apache_hive}, and its query language HiveQL is possible. Because of its HDFS underpinnings it can function as both input and output for MapReduce jobs.
\\HBase partitions data in ranges like BigTable and replicates updates in master-slave or multi-master fashion. Read-requests are not distributed however, as rows are only serviced by one server. The replicas are solely intended for failure recovery.
...
In conclusion, HBase offers flexible data-modeling and is especially useful when there is already a HDFS dataset and when MapReduce compatibility is desired. It scales well to very large datasets and offers excellent concurrency control and strong consistency.

\section{NewSQL}
\subsection{VoltDB}

VoltDB is a relational in-memory distributed database, aiming to offer the guarantees of classical SQL-stores and the scaling of NoSQL systems at very high speeds\cite{stonebraker2013voltdb}.
...
\\Contrary to the options discussed before, Impala doesn't partition rows by default. It does offer the possibility and leaves the user free to partition data should the data size call for it\cite{impala_partitioning}.
